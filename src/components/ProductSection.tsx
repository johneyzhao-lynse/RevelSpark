        ...
        <div className="flex flex-col lg:flex-row gap-12 items-center">
            ...
            <div className="glass-card p-8 rounded-3xl animate-fade-in-up opacity-0-initial delay-300 shadow-premium hover:shadow-premium-hover transition-all duration-500">
                ...
                {(Object.keys(tabs) as Array<keyof typeof tabs>).map((tab) => (
                  <button
                    ...
                    className={`px-5 py-3 text-sm font-semibold whitespace-nowrap transition-all rounded-t-xl relative ${activeTab === tab
                        ? 'text-black bg-gray-50 shadow-inner-soft'
                        : 'text-text-tertiary hover:text-black hover:bg-gray-100/40'
                      }`}
                    onClick={() => setActiveTab(tab)}
                  >
                    <div className="flex items-center gap-2 relative z-10">
                      {React.cloneElement(tabs[tab].icon as React.ReactElement, { size: 18, className: activeTab === tab ? 'text-black' : 'text-text-tertiary' })}
                      <span>{tabs[tab].title}</span>
                    </div>
                    {activeTab === tab && (
                      <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-gray-400 to-gray-600 rounded-full"></div>
                    )}
                  </button>
                ))}
              </div>
              ...
              <div className="tab-content">
                <div className="flex items-start gap-4 mb-6">
                  <div className="rounded-2xl p-3 bg-gradient-to-br from-gray-100 to-gray-200 shadow-inner-soft">
                    {React.cloneElement(tabs[activeTab].icon as React.ReactElement, { size: 24, className: 'text-black' })}
                  </div>
                  <div>
          ...
          <div className="w-full lg:w-1/2 order-1 lg:order-2">
            <div className="relative animate-scale-in opacity-0-initial">
              ...
              <div className="absolute -right-20 top-0 w-64 h-64 bg-gray-200 rounded-full blur-3xl opacity-20 z-0 animate-float"></div>
              <div className="absolute -left-20 bottom-20 w-64 h-64 bg-gray-300 rounded-full blur-3xl opacity-20 z-0 animate-float-delayed"></div>
              ...
              <div className="relative z-10 flex justify-center items-center">
                <div className="relative w-3/4 aspect-square">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-tr from-gray-600 to-gray-800 rounded-3xl transform -rotate-3 overflow-hidden shadow-glow">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center p-8">
                      <div className="w-full h-full bg-white/95 backdrop-blur-sm rounded-xl flex flex-col items-center justify-center shadow-inner">
                        ...
                        <div className="w-24 h-24 bg-black rounded-full mb-6 flex items-center justify-center shadow-lg">
                          <div className="w-8 h-8 bg-white rounded-sm"></div>
                        </div>
                        <div className="w-10 h-10 bg-gray-800 rounded-full absolute bottom-10 right-10 flex items-center justify-center shadow-md">
                          <div className="w-6 h-6 bg-white rounded-full"></div>
                        </div>
                        <div className="w-3/4 h-1 bg-gray-200 rounded-full mt-2"></div>
                        <div className="w-1/2 h-1 bg-gray-200 rounded-full mt-3"></div>
                        <div className="font-bold text-black mt-6">DtingNote</div>
                        <div className="text-xs text-text-tertiary">灵光记</div>
                      </div>
                    </div>
                  </div>
                </div>
